...
class RedBlackTree {
    ...
    while (x != root && x->color == 0) {
      if (x == x->parent->left) {
        ...
        if (s->left->color == 0 && s->right->color == 0) {
      	...
      	template <class T, class node_pointer> class map_iterator;
  ...
  void deleteNodeHelper(NodePtr node, int key) {
    ...
    y 	template <class T, class node_pointer> class map_iterator;
	template <class T> class reverse_iterator;
} = z;
...
// namespace ft
// {
//     template <typename T, typename Key, class Compare, class Alloc>
